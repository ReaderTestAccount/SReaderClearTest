...
import com.sreader.BookPageSpliter.TextWidthMeasurer;
...
public class RunRead extends Activity
{
	...
	private void initialVariable()
	{
		...
		if (Preferences.getColorSheme() != 0)
			setColorSheme();

		if (NAME_FORGET_FILE.endsWith("html") || NAME_FORGET_FILE.endsWith("htm") || NAME_FORGET_FILE.endsWith("txt"))
			setHidenButtToolbar();
	}
	...
	public Dialog onCreateDialog(int id)
	{
		...
		case ID_DIALOG_NOTCONTENT:
			...
			dialogContent.setTitle("Ошибка!").setMessage("Структура документа повреждена").setPositiveButton("Ok", new OnClickListener() {
	...
	public void onClickListChapter(View v)
	{
		...
		for (Chapter cut_ch : book)
			chapters.add(cut_ch.getName());
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		...
		case KeyEvent.KEYCODE_BACK:
			if (CURRENT_VISIBLE == Constants.VISIBLE_RUNTEXT)
			{
				if (linlaytoolbar.isShown())
					setActiveDeactiveToolBar();
				else if (linlaybottolbar.isShown())
					setActiveDeactiveContext();
				else
					finish();
			}
	...
	public void addBookmark(View v)
	{
		if (!addBookmark)
		{
			...
			dbBMHelper.addItemBookmark(new ItemBookmark(new String[] { Utils.getCurrentDate(), ID_ITEM, curchapter.getName(), phrase,
					String.valueOf(progres_pb_hor), String.valueOf(id_chapter), String.valueOf(id_word), String.valueOf(countWords), NAME_FORGET_FILE,
					DIR_FORGET_FILE }));
	...
	public void onLowMemory()
	{
		...
		Log.i(getClass().getSimpleName(), "OnLowMemory invoked");
		super.onLowMemory();
	}

	public class addTextToArr extends AsyncTask<String, String, Void>
	{
		Context context;
		...
		private TextWidthMeasurer createTextWidthMeasurer()
		{
			TextView textview = mAdapter.getTextView();
			final int curDisplayWidth = Utils.getWidthDisplay(context) - textview.getPaddingLeft() - textview.getPaddingLeft();
			final Paint textPaint = textview.getPaint();

			TextWidthMeasurer merger = new TextWidthMeasurer() {
				private final char[] oneCharBuffer = new char[1];
				
				public boolean isSinglelineText(String text)
				{
					if(Config.ASSERT_ENABLED)	//Make check here for optimization, to avoid method call
						AssertExeption._assertParamNull(text, "text");

					boolean res = Utils.willDrawSinglelineText(textPaint, text, curDisplayWidth);
					return res;
				}

				public float[] getCharactersWidths(String text)
				{
					if(Config.ASSERT_ENABLED)	//Make check here for optimization, to avoid method call
						AssertExeption._assertParamNull(text, "text");
				...
				{
					oneCharBuffer[0] = character;
					return textPaint.measureText(oneCharBuffer, 0, 1);
				}
				
				public int getMaxPageWidth()
				{
					return curDisplayWidth;
				}
			};
		...
		{
			Log.e("Paging ","start!!!!!!!!!!");
			TextWidthMeasurer merger = createTextWidthMeasurer();
			BookPageSpliter pageSpliter = new BookPageSpliter(book, words, setChapters, merger);
			pageSpliter.split();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (resultCode == RESULT_OK)
		{
			...
			/*
			 * if (pausedRead) { linlaytoolbar.setVisibility(View.INVISIBLE);
			 * linlaybottolbar.setVisibility(View.INVISIBLE);
			 * linlayprogressbar_hor.setVisibility(View.VISIBLE);
			 * linlayspeedtext.setVisibility(View.VISIBLE);
			 * handler.removeCallbacks(updateTimeTask);
			 * handler.postDelayed(updateTimeTask, time); pausedRead=false; }
			 */
		}
		else
		{
	...
	public void printAddBookmarkDialog(String bm_add)
	{
		...
		AlertDialog.Builder dialogFont = new AlertDialog.Builder(this);
		dialogFont.setTitle("Информация").setMessage("Закладка \"" + bm_add + "\" добавлена")
	...
	private void setPreferSh_O()
	{
		...
		final Paint textPaint = text_torun.getPaint();
		while (Utils.willDrawSinglelineText(textPaint, sh_o, Utils.getWidthDisplay(this)))
		{
			sh_o += "a";
		}
		DEFAULT_SH_V = sh_o.length() - 1;
		if (Preferences.getSh_O() == 0)
			Preferences.setSh_O(DEFAULT_SH_V);
