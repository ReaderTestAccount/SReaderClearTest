...
public class FB2ParserSaxHandler extends DefaultHandler
{
	String ElementName = "";
	private Book book;
	...
	private StringBuilder ch_str = new StringBuilder(64);
	boolean descript = false;
	boolean sect = false;

	public FB2ParserSaxHandler(Book books)
	{
		this.book = books;
	}
	...
	public void characters(char[] ch, int start, int length) throws SAXException
	{
		if (descript)
		{
			if (ElementName.equalsIgnoreCase("p") || ElementName.equalsIgnoreCase("i"))
				ch_str.append(new String(ch, start, length) + "\n");
			else if (ElementName.equalsIgnoreCase("genre"))
				book.genres.add(new String(ch, start, length));
			else if (ElementName.equalsIgnoreCase("first-name"))
				authors.setFirst_name(new String(ch, start, length));
			else if (ElementName.equalsIgnoreCase("last-name"))
				authors.setLast_name(new String(ch, start, length));
			else if (ElementName.equalsIgnoreCase("book-title"))
				book.setBook_title(new String(ch, start, length));
		}
		else if ( (!descript) && sect )
		{
			// if (ElementName.equalsIgnoreCase("title"))
			if (ElementName.equalsIgnoreCase("p") || ElementName.equalsIgnoreCase("i"))
			{
				ch_str.append(ch, start, length);
				ch_str.append("\n");
			}
